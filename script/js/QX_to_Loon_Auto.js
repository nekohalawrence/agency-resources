/** 测试  https://raw.githubusercontent.com/chengkongyiban/Loon/main/Loon-Gallery/Rewrite_to_Loon.plugin
...
if (loonua) {
    ...
    req = yurl.replace(/qx\.stoverride$|qx\.stoverride\?.*/,'');
    if (yurl.indexOf("qx.stoverride?") != -1){
        urlArg = "?" + yurl.split("qx.stoverride?")[1];
    }else{urlArg = ""};
};
...
if(body == null || body == ""){
    ...
    console.log("QX转换：未获取到body的链接为" + yurl)
	$notification.post("QX转换：未获取到body","请检查网络及节点是否畅通\n" + "源链接为" + yurl,"认为是bug?点击通知反馈",{url:"https://t.me/zhangpeifu"})
 $done({ response: { status: 404 ,body:{} } });
    }else{
    console.log("QX转换：未获取到body的链接为" + yurl)
    $notification.post("QX转换：未获取到body","请检查网络及节点是否畅通\n" + "源链接为" + yurl,"认为是bug?点击通知反馈","https://t.me/zhangpeifu")
 $done({ response: { status: 404 ,body:{} } });
    }    //识别客户端通知
}else{//以下开始重写及脚本转换
